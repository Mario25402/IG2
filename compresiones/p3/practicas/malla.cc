...
void Malla3D::draw(bool puntos, bool alambre, bool solido, int color)
{
   // Modo de visualización //

   setColor(puntos, alambre, solido, color);
...
void Malla3D::setColor(bool puntos, bool alambre, bool solido, int color)
{
   ...
   if (solido and !oldSolido){
      ...
      for (int i = 0; i < v.size(); i++){
         if (color == 1)
            c[i] = Tupla3f(1.0f, 0.0f, 0.0f);

         else if (color == 2)
            c[i] = Tupla3f(0.0f, 1.0f, 0.0f);

         else if (color == 3)
            c[i] = Tupla3f(0.0f, 0.0f, 1.0f);
